...
This is an implementation of the sql-mandelbrot-benchmark using CedarDB.
It computes the classic Mandelbrot set in plain SQL — no loops, no procedural code, just pure SQL.

CedarDB speaks the PostgreSQL wire protocol, so we connect with psycopg.

Run CedarDB with Docker:
    docker run --rm -p 5432:5432 -e CEDAR_PASSWORD=postgres cedardb/cedardb:latest
...
def run_cedarbrot(width: int, height: int, max_iterations: int) -> np.ndarray:
    ...
    GROUP BY x, y;
    """
    # No ORDER BY: the result is scattered into the image by (x, y) below, so sorting
    # server-side would only add work (same optimization used for the ClickHouse entry).
